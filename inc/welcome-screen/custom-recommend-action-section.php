 ...
 * @access public
 */
class Maha_Customize_Section_Recommend extends WP_Customize_Section {
	/**
	 ...
	 * @var    string
	 */
	public $type = 'maha-recomended-section';
	/**
	...
	public function json() {
		...
		foreach ( $maha_required_actions as $key => $maha_required_action ) {
			if ( $maha_required_action['check'] ) {
				continue;
			}

			$maha_required_action['index'] = $key + 1;

			if ( isset($maha_required_action['plugin_slug']) ) {
				$active = $this->check_active( $maha_required_action['plugin_slug'] );
				$maha_required_action['url']    = $this->create_action_link( $active['needs'], $maha_required_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$maha_required_action['class'] = 'active';
				}else{
					$maha_required_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$maha_required_action['button_class'] = 'install-now button';
						$maha_required_action['button_label'] = __( 'Install', 'marigold' );
						break;
					case 'activate':
						$maha_required_action['button_class'] = 'activate-now button button-primary';
						$maha_required_action['button_label'] = __( 'Activate', 'marigold' );
						break;
					case 'deactivate':
						$maha_required_action['button_class'] = 'deactivate-now button';
						$maha_required_action['button_label'] = __( 'Deactivate', 'marigold' );
						break;
				}

			}
			$formatted_array[] = $maha_required_action;
		}
		$json['required_actions'] = $formatted_array;
		$json['total_actions'] = count($maha_required_actions);
			...
			<div class="recomended-actions_container" id="plugin-filter">
				<# if( data.required_actions.length > 0 ){ #>
					<# for (action in data.required_actions) { #>
						<div class="maha-recommeded-actions-container" data-index="{{ data.required_actions[action].index }}">
							<# if( !data.required_actions[action].check ){ #>
								<div class="maha-recommeded-actions">
				...
				<# if( data.required_actions.length == 0 ){ #>
					<p class="succes">
						...
						<# } #>
						<# if( data.wp_review ){ #>
							<a href="https://wordpress.org/support/theme/{{ data.theme_slug }}/reviews/#new-post" class="button button-primary maha-wordpress"><span class="dashicons dashicons-wordpress"></span>Review this theme on w.org</a>
						<# } #>
					</p>
				<# }else{ #>
					<p class="succes hide">
